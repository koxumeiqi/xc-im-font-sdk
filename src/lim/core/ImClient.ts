...
export interface IListener {
  ...
  onP2pMsgReciveMessage(e: any): void;// 消息已读，收到ACK后执行对应回调
...
export class ImClient {
  ...
  public async init(httpUrl: string, appId: number, userId: string, userSign: string, listeners: any, callback: (sdk: ImClient) => void) {
    ...
    if (success) {
      ...
      conn.onmessage = (evt) => {
        ...
        if (command === MessageCommand.MSG_RECIVE_ACK) {
          //单聊消息收发
          if (typeof imClient.listeners.onP2PMessage === "function") {
            imClient.listeners.onP2PMessage(msgBody); // 消息已经发送了执行回调
          }
        } else if (command === MessageCommand.MSG_ACK) {
          if (typeof imClient.listeners.onP2pMsgReciveMessage === "function") {
            imClient.listeners.onP2pMsgReciveMessage(msgBody);
          }
  ...
  public buildMessagePack(command: number, messagePack: any) {
    ...
    pack.int32(command).int32(this.version).int32(this.clientType)
      .int32(0x0) // 解析数据类型。0:json 1:protobuf
      .int32(this.appId)
  ...
  public createGroupTextMessage(to: string, text: string) {
    ...
    messagePack.buildGroupTextMessagePack(this.userId, to, text);
    return messagePack;
  }
